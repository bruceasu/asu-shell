...
import asu.sh.History;
import asu.sh.Jobs;
import asu.sh.MultiLineBufferPrintStream;
import asu.sh.PagedPrintStream;
...
public abstract class Command {
    ...
    protected final PrintStream out() {
        if (this.out == null) {
            this.out = System.out;
            if (System.getProperty("asu.shell.buffer").equals("true")) {
                this.out = new MultiLineBufferPrintStream(this.out);
            }
            if (System.getProperty("asu.shell.page").equals("true")) {
    ...
    protected final PrintStream err() {
        if (this.err == null) {
            this.err = System.err;
            if (System.getProperty("asu.shell.buffer").equals("true")) {
                this.err = new MultiLineBufferPrintStream(this.err);
            }
            if (System.getProperty("asu.shell.page").equals("true")) {
    ...
    public static void connectOutputToInput(Object paramObject1, Object paramObject2)
            throws IOException {
        ...
        ((Command) paramObject1).out = new PrintStream(localPipedOutputStream);
        ((Command) paramObject1).redirectOut = true;
        ((Command) paramObject2).in = localPipedInputStream;
        ((Command) paramObject2).redirectIn = true;
    }

    protected void printHistory(int paramInt) {
        History.getInstance().printLast(paramInt, out());
    }
